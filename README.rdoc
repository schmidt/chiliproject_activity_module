= Redmine/ChiliProject Activity Module Plugin
...
It therefore simply restores the default Redmine/ChiliProject behaviour for all
existing projects. You may then selectively deactivate the activity module in
single projects selectively.
...
It was developed on ChiliProject 1.1 and then backported to Redmine 0.9.
Currently it is only used with ChiliProject, so it may not run on Redmine
anymore. Use at own risk.
...
https://github.com/finnlabs/redmine_activity_module/raw/master/doc/screen-capture.png
...
Afterwards simple remove the plugin's directory from your vendor/plugins
directory.
